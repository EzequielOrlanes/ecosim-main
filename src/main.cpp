...
// void growing_plant(uint32_t i, uint32_t j, std::vector<std::vector<entity_t>> updated_grid) {
...
//                                     if (target_entity.type == entity_type_t::vacant && target_entity.type != entity_type_t::carnivore) {
//                                         // Move o herbívoro para a célula vizinha
//                                         updated_entity.type = entity_type_t::vacant;
    ...
    std::vector<std::vector<entity_t>> updated_grid = entity_grid;
        for (uint32_t i = 0; i < NUM_ROWS; ++i) {
            for (uint32_t j = 0; j < NUM_ROWS; ++j) {
                ...
                if (current_entity.type == entity_type_t::vacant) continue;
                // Update the age
                updated_entity.age++;
                ...
                if (current_entity.type == plant) {
                    cout << "Oi";
                    // thread new_plants_thread(growing_plant, i,j, updated_grid);
                    // new_plants_thread.join();
                } 
                // // Implement movementing to herbivores
                // if (current_entity.type == herbivore) {
                //     thread new_herbivores_movements(herbiviral_movement,i, j, updated_grid[i][j], updated_entity, current_entity );
                //     new_herbivores_movements.join();
                // }
                // // Implement eating for herbivores
                // if (current_entity.type == herbivore) {
                //      thread  new_herbiviral_eating(herbiviral_eating, i, j,updated_grid[i][j], updated_entity, current_entity);
                //      new_herbiviral_eating.join();
                // }
                // // Implement reproduction and energy update for herbivores
                // if (current_entity.type == herbivore) {
                //     thread new_herbiviral_energy_reproduction(herbiviral_energy_reproduction,i,j,updated_grid[i][j], updated_entity, current_entity);
                //     new_herbiviral_energy_reproduction.join();
                // }
                // Implement movement for carnivores
                // if (current_entity.type == carnivore) {
                //      std::thread new_carnivore_movement(carnivore_movement,i, j,updated_grid, updated_entity, current_entity);
                //      new_carnivore_movement.join();
                // }
                // // Implement eating for carnivores
                // if (current_entity.type == carnivore) {
                //     thread new_carnivore_eating (carnivore_eating,i, j,updated_grid[i][j], updated_entity, current_entity);
                //     new_carnivore_eating.join();
                // }
                // // Implement reproduction and energy update for carnivores
                // if (current_entity.type == carnivore) {
                //     thread new_carnivore_energy_reproduction(carnivore_energy_reproduction,i,j,updated_grid, updated_entity, current_entity);
